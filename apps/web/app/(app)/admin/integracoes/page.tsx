...
export default async function AdminIntegracoesPage() {
  ...
  return <main className="mx-auto max-w-6xl space-y-6 p-4 sm:p-6"><div><h1 className="font-heading text-xl font-medium">Integrações</h1><p className="text-sm text-muted-foreground">Defina uma conexão universal para toda a empresa ou permita contas individuais por usuário.</p></div><IntegrationsAdmin integrations={integrations} users={users.map(({ id, name, email, active }) => ({ id, name, email, active }))} appUrl={appUrl} /></main>;
}

...
export default async function IntegracoesPage({ searchParams }: { searchParams: Promise<{ connected?: string; integrationError?: string }> }) {
  ...
  return <main className="h-full overflow-y-auto"><div className="mx-auto max-w-6xl space-y-6 p-4 sm:p-6"><div><h1 className="font-heading text-xl font-medium">Minhas integrações</h1><p className="text-sm text-muted-foreground">Conecte suas próprias contas. O agente poderá consultar as plataformas durante a conversa.</p></div><IntegrationWizardList initialIntegrations={integrations} notice={{ success: params.connected, error: params.integrationError }} /></div></main>;
}

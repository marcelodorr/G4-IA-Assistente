...
export default async function NewChatPage({ searchParams }: { searchParams: Promise<{ prompt?: string }> }) {
  ...
  const [assistentes, settings, access, integrations, params] = await Promise.all([
  ...
  return <NewChat assistants={assistants} defaultModel={defaultModel} models={models} initialPrompt={initialPrompt} integrationSuggestions={integrationSuggestions} />;
}

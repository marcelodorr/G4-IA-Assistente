...
type IntegrationAdmin = {
  ...
  setupSteps: string[]; docsUrl: string; individualOnly?: boolean; active: boolean; connectionMode: "individual" | "universal"; universalConnected: boolean; universalAccountLabel: string | null; clientId: string; secretConfigured: boolean; userIds: string[];
};
  ...
  return (
    <Card className="overflow-hidden">
      ...
      <CardContent className="space-y-5">
        ...
        <div className="space-y-2 rounded-lg border p-4"><Label>Modo da conexão</Label><Select value={integration.individualOnly ? "individual" : connectionMode} disabled={integration.individualOnly} onValueChange={(value) => setConnectionMode(value as "individual" | "universal")}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{!integration.individualOnly && <SelectItem value="universal">Universal — uma conta para toda empresa</SelectItem>}<SelectItem value="individual">Individual — cada usuário conecta sua conta</SelectItem></SelectContent></Select><p className="text-xs text-muted-foreground">{integration.individualOnly ? "Esta integração usa a agenda e a identidade individual de cada usuário." : connectionMode === "universal" ? "Todos os usuários usarão a conexão configurada por um administrador." : "Cada usuário autorizado configura e utiliza somente a própria conta."}</p></div>

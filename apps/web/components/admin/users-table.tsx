...
export function UsersTable({ users, assistants, enabledModels, globalQuotas }: {
  users: UserRow[];
  assistants: Array<{ id: string; name: string; active: boolean }>;
        ...
        <TableBody>
          {users.map((user) => (
            <TableRow key={user.id}>
              ...
              <TableCell><div className="flex flex-wrap gap-1"><UserAccessDialog user={user} assistants={assistants} enabledModels={enabledModels} globalQuotas={globalQuotas} onSaved={() => router.refresh()} /><Button variant="outline" size="sm" disabled={busy === user.id} onClick={() => update(user, { active: !user.active }, user.active ? "Usuário desativado" : "Usuário ativado")}>{user.active ? "Desativar" : "Ativar"}</Button><Button variant="ghost" size="sm" disabled={busy === user.id} onClick={() => update(user, { revokeSessions: true }, "Sessões encerradas")}>Encerrar sessões</Button></div></TableCell>
            </TableRow>
          ))}

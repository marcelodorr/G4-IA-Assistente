...
import { ImagePlus, Plug, Sparkles } from "lucide-react";
...
export function ChatCapabilitySelectors({ integrations, skills, controls, onChange, defaultIntegrationId }: { integrations: IntegrationOption[]; skills: SkillOption[]; controls: ChatControls; onChange: (value: ChatControls) => void; defaultIntegrationId?: string | null }) {
  const toggle = (key: "selectedIntegrationIds" | "selectedSkillIds", id: string) => onChange({ ...controls, [key]: controls[key].includes(id) ? controls[key].filter((value) => value !== id) : [...controls[key], id] });
  return <div className="mb-2 flex max-w-full gap-2 overflow-x-auto pb-1 sm:flex-wrap sm:overflow-visible sm:pb-0">
    <Button
      className={cn("h-9 shrink-0 sm:h-7", controls.generationMode === "image" && "border-fuchsia-400 bg-fuchsia-600 text-white hover:bg-fuchsia-700")}
      type="button"
      size="sm"
      variant={controls.generationMode === "image" ? "default" : "outline"}
      aria-pressed={controls.generationMode === "image"}
      onClick={() => onChange({ ...controls, generationMode: controls.generationMode === "image" ? "chat" : "image" })}
    >
      <ImagePlus />{controls.generationMode === "image" ? "Imagem ativada" : "Criar imagem"}
    </Button>

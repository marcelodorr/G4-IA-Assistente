...
type IntegrationItem = {
  ...
  connectionMode: "individual" | "universal"; managedByCompany: boolean; canConfigure: boolean;
};
...
function Wizard({ integration, open, onOpenChange, onChanged }: { integration: IntegrationItem; open: boolean; onOpenChange: (open: boolean) => void; onChanged: () => Promise<void> }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle>{integration.connected ? `Como usar ${integration.name}` : `Conectar ${integration.name}`}</DialogTitle><DialogDescription>Etapa {step} de 3 — não é necessário decorar comandos técnicos.</DialogDescription></DialogHeader>
    {step === 1 && <div className="space-y-4">{integration.managedByCompany && <div className="rounded-lg border border-primary/30 bg-primary/5 p-3 text-sm"><span className="font-medium">Integração universal.</span> {integration.connected ? "A conta corporativa já está pronta para todos os usuários." : integration.canConfigure ? "Conecte a conta corporativa; ela ficará disponível para toda a empresa." : "A conexão é gerenciada pelo administrador e ainda não está pronta."}</div>}<div className="rounded-lg border p-4"><h3 className="font-medium">O que você consegue consultar</h3><ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-muted-foreground">{integration.capabilities.map((item) => <li key={item}>{item}</li>)}</ul></div><div><h3 className="text-sm font-medium">Exemplos do que escrever no chat</h3><div className="mt-2 space-y-2">{integration.examplePrompts.slice(0, 3).map((prompt) => <p key={prompt} className="rounded-lg bg-secondary/50 p-2.5 text-sm">“{prompt}”</p>)}</div></div><div className="rounded-lg border border-amber-500/30 p-3"><h3 className="text-sm font-medium">O que esta integração não faz</h3><ul className="mt-1 list-disc space-y-1 pl-5 text-xs text-muted-foreground">{integration.limitations.map((item) => <li key={item}>{item}</li>)}</ul></div>{!integration.managedByCompany && <div><h3 className="text-sm font-medium">Como conectar — passo a passo</h3><ol className="mt-2 list-decimal space-y-2 pl-5 text-sm text-muted-foreground">{integration.userSteps.map((item) => <li key={item}>{item}</li>)}</ol></div>}<div className="rounded-lg bg-primary/5 p-3 text-sm"><span className="font-medium">Importante:</span> depois de conectar, basta escrever normalmente. O agente chama {integration.name} sozinho.</div></div>}
    {step === 2 && <div className="space-y-4">{integration.authType === "oauth" ? <div className="rounded-lg border p-4 text-sm"><p>Você será direcionado para {integration.name} para escolher a conta e autorizar acesso somente de leitura.</p><p className="mt-2 text-muted-foreground">A Sequor não recebe sua senha. Tokens de acesso e renovação são armazenados criptografados.</p></div> : <div className="space-y-2"><Label htmlFor={`token-${integration.id}`}>{integration.tokenLabel ?? `Token pessoal do ${integration.name}`}</Label><Input id={`token-${integration.id}`} type="password" value={token} onChange={(event) => setToken(event.target.value)} placeholder={integration.tokenPlaceholder ?? "Cole seu token pessoal"} autoComplete="off" /><p className="text-xs text-muted-foreground">{integration.tokenHelp ?? "O token será validado e armazenado criptografado."}</p></div>}<a className="text-sm text-primary hover:underline" href={integration.docsUrl} target="_blank" rel="noreferrer">Ver documentação oficial</a></div>}
    ...
    <DialogFooter>{step > 1 && step < 3 && <Button variant="outline" onClick={() => setStep(step - 1)}>Voltar</Button>}{step === 1 && !integration.connected && integration.canConfigure && <Button onClick={() => setStep(2)}>Continuar para conectar</Button>}{step === 1 && !integration.connected && !integration.canConfigure && <Button onClick={() => onOpenChange(false)}>Entendi</Button>}{step === 2 && <Button disabled={busy || (integration.authType === "token" && token.trim().length < 20)} onClick={() => void connect()}>{busy ? "Conectando..." : integration.authType === "oauth" ? `Autorizar no ${integration.name}` : "Validar e conectar"}</Button>}{step === 3 && <Button onClick={() => onOpenChange(false)}>Concluir</Button>}</DialogFooter>
...
export function IntegrationWizardList({ initialIntegrations, notice }: { initialIntegrations: IntegrationItem[]; notice?: { success?: string; error?: string } }) {
  ...
  return <>
    <Card className="mb-5 border-primary/30 bg-primary/5"><CardHeader><CardTitle>Como usar no chat — sem comandos</CardTitle><CardDescription>Você não precisa escrever “chamar integração” nem escolher uma ferramenta.</CardDescription></CardHeader><CardContent><ol className="grid gap-3 text-sm sm:grid-cols-3"><li className="rounded-lg bg-background p-3"><strong className="block">1. Verifique a conexão</strong><span className="text-muted-foreground">Pode ser sua conta ou uma conta universal da empresa.</span></li><li className="rounded-lg bg-background p-3"><strong className="block">2. Escreva normalmente</strong><span className="text-muted-foreground">Ex.: “Quais reuniões tenho hoje?”</span></li><li className="rounded-lg bg-background p-3"><strong className="block">3. O agente consulta</strong><span className="text-muted-foreground">Um aviso aparece enquanto a plataforma é consultada.</span></li></ol></CardContent></Card>
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{integrations.map((item) => <Card key={item.id} className="overflow-hidden"><div className="h-1" style={{ backgroundColor: item.color }} /><CardHeader><div className="flex items-start justify-between gap-2"><div><CardTitle>{item.name}</CardTitle><CardDescription>{item.description}</CardDescription></div><div className="flex flex-col items-end gap-1"><Badge variant={item.connected ? "default" : "outline"}>{item.connected ? "Conectada" : "Disponível"}</Badge>{item.managedByCompany && <Badge variant="outline">Empresa</Badge>}</div></div></CardHeader><CardContent className="space-y-4"><div><p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Você pode</p><ul className="mt-1 list-disc space-y-1 pl-5 text-sm">{item.capabilities.slice(0, 3).map((capability) => <li key={capability}>{capability}</li>)}</ul></div><div className="rounded-lg bg-secondary/50 p-3 text-sm"><span className="block text-xs font-medium text-muted-foreground">Exemplo no chat</span>“{item.examplePrompts[0]}”</div>{item.connected && <div className="text-sm"><p className="font-medium">{item.accountLabel}</p>{item.lastUsedAt && <p className="text-xs text-muted-foreground">Último uso: {new Date(item.lastUsedAt).toLocaleString("pt-BR")}</p>}{item.lastError && <p className="mt-1 text-xs text-destructive">{item.lastError}</p>}</div>}<div className="flex flex-wrap gap-2"><Button className="flex-1" variant={item.connected ? "outline" : "default"} onClick={() => setSelected(item)}><PlugIcon />{item.connected ? "Ver exemplos" : item.canConfigure ? "Conectar passo a passo" : "Ver detalhes"}</Button>{item.connected && <Button asChild className="flex-1"><Link href={`/?prompt=${encodeURIComponent(item.examplePrompts[0])}`}><MessageCircleIcon />Usar no chat</Link></Button>}{item.connected && (!item.managedByCompany || item.canConfigure) && <Button size="icon" variant="ghost" aria-label={`Desconectar ${item.name}`} onClick={() => void disconnect(item)}><UnplugIcon /></Button>}</div></CardContent></Card>)}</div>

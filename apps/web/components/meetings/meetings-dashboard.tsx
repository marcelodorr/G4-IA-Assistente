...
import { Bot, CalendarDays, ExternalLink, Loader2, MessageSquareText, Mic, Plus, RefreshCw, Send, Sparkles, Square, Video } from "lucide-react";
...
export function MeetingsDashboard() {
  ...
  async function createMeeting() {
    ...
    setCreating(true);
    const response = await fetch("/api/meetings", { method: "POST", body: JSON.stringify({ title: newMeetingTitle, assistantId: assistantId || null }) });
    const body = await response.json().catch(() => ({}));
    setCreating(false);
    if (!response.ok) return toast.error(body.error ?? "Não foi possível criar a reunião");
    setNewMeetingTitle(""); setNewMeetingOpen(false); setSelectedId(body.id);
  ...
  return <div className="grid min-h-full gap-4 p-4 lg:grid-cols-[19rem_minmax(0,1fr)] sm:p-6">
    <aside className="space-y-3">
      <div className="flex items-center justify-between"><div><h1 className="font-heading text-xl font-medium">Reuniões</h1><p className="text-xs text-muted-foreground">Copiloto universal · Teams opcional</p></div><div className="flex gap-2"><Dialog open={newMeetingOpen} onOpenChange={setNewMeetingOpen}><DialogTrigger asChild><Button size="icon" disabled={captureStatus !== "idle"} aria-label="Nova reunião"><Plus /></Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>Nova reunião</DialogTitle><DialogDescription>Crie um acompanhamento para qualquer chamada no navegador, sem integrar a plataforma de reunião.</DialogDescription></DialogHeader><div className="space-y-2"><Label htmlFor="new-meeting-title">Nome da reunião</Label><Input id="new-meeting-title" value={newMeetingTitle} onChange={(event) => setNewMeetingTitle(event.target.value)} placeholder="Ex.: Call comercial com Acme" maxLength={200} onKeyDown={(event) => { if (event.key === "Enter") void createMeeting(); }} /></div><DialogFooter showCloseButton><Button disabled={creating || !newMeetingTitle.trim()} onClick={() => void createMeeting()}>{creating ? "Criando..." : "Criar reunião"}</Button></DialogFooter></DialogContent></Dialog><Button size="icon" variant="outline" disabled={syncing} onClick={() => void loadAgenda(true)} aria-label="Atualizar agenda">{syncing ? <Loader2 className="animate-spin" /> : <RefreshCw />}</Button></div></div>
    ...
    <section className="min-w-0">{!state ? <div className="grid h-full min-h-96 place-items-center rounded-xl border border-dashed text-center text-muted-foreground"><div><Video className="mx-auto mb-3 size-10" /><p>Selecione uma reunião para acompanhar.</p></div></div> : <div className="flex h-[calc(100dvh-3rem)] min-h-[38rem] flex-col overflow-hidden rounded-xl border bg-card lg:h-[calc(100dvh-3rem)]">
      <header className="flex flex-col gap-3 border-b p-4 xl:flex-row xl:items-center xl:justify-between"><div className="min-w-0"><div className="flex items-center gap-2"><Badge variant={captureStatus === "capturing" ? "default" : "outline"}>{captureStatus === "capturing" ? "Transcrevendo ao vivo" : captureStatus === "connecting" ? "Conectando áudio" : state.meeting.status === "live" ? "Acompanhamento pausado" : "Pronta para acompanhar"}</Badge><span className="text-xs text-muted-foreground">{time(state.meeting.startsAt)}–{time(state.meeting.endsAt)}</span></div><h2 className="mt-1 truncate text-lg font-medium">{state.meeting.title}</h2></div><div className="flex flex-wrap items-center gap-2"><Select value={assistantId} onValueChange={setAssistantId} disabled={captureStatus !== "idle"}><SelectTrigger className="w-56"><Bot /><SelectValue placeholder="Escolha um assistente" /></SelectTrigger><SelectContent>{assistants.map((assistant) => <SelectItem key={assistant.id} value={assistant.id}>{assistant.name}</SelectItem>)}</SelectContent></Select>{state.meeting.joinUrl && <Button asChild variant="outline"><a href={state.meeting.joinUrl} target="_blank" rel="noreferrer"><ExternalLink />Entrar no Teams</a></Button>}{captureStatus === "capturing" ? <Button variant="destructive" onClick={() => void stopCapture()}><Square />Encerrar captura</Button> : <Button disabled={!assistantId || captureStatus === "connecting" || state.meeting.status === "cancelled"} onClick={() => void beginCapture()}>{captureStatus === "connecting" ? <Loader2 className="animate-spin" /> : <Mic />}Compartilhar áudio</Button>}</div></header>
      {captureStatus === "idle" && <div className="border-b bg-primary/5 px-4 py-3 text-xs text-muted-foreground"><strong className="text-foreground">Como iniciar:</strong> abra a chamada em outra aba, clique em “Compartilhar áudio”, escolha a aba da reunião e marque a opção de compartilhar o áudio. O navegador também solicitará acesso ao seu microfone.</div>}

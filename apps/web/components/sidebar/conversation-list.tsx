...
import { ChevronDown, ChevronRight, Folder, FolderInput, GripVertical, LogOut, MessageSquare, Plug, Settings2, Shield, Trash2, UserRound } from "lucide-react";
...
export function ConversationList({ conversations, projects, user, usage, liveUsage }: { conversations: ConversationRow[]; projects: ProjectRow[]; user: SidebarUser; usage: Usage | null; liveUsage: boolean }) {
  ...
  const [moveConversationTarget, setMoveConversationTarget] = useState<ConversationRow | null>(null);
  ...
  async function moveConversation(projectId: string | null, conversationId: string | null | undefined = draggingId) {
  ...
  function conversationItem(conversation: ConversationRow) {
    ...
    return <div key={conversation.id} draggable onDragStart={(event) => { event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/plain", conversation.id); setDraggingId(conversation.id); }} onDragEnd={() => { setDraggingId(null); setDropTarget(null); }} className={cn("group/chat flex items-center rounded-lg transition", pathname === href ? "bg-primary/12 text-primary" : "text-muted-foreground hover:bg-accent hover:text-foreground", busy === conversation.id && "opacity-50", draggingId === conversation.id && "opacity-40")}><GripVertical className="ml-1 hidden size-3 shrink-0 cursor-grab opacity-0 group-hover/chat:opacity-50 md:block" /><Link href={href} className="min-w-0 flex-1 truncate py-2 pl-2 text-sm md:py-1.5 md:pl-1"><MessageSquare className="mr-1.5 inline size-3.5" />{conversation.title || "Nova conversa"}</Link><button type="button" onClick={() => setMoveConversationTarget(conversation)} aria-label={`Mover ${conversation.title ?? "chat"}`} className="rounded p-2 text-muted-foreground hover:bg-primary/10 hover:text-primary md:p-1 md:opacity-0 md:group-hover/chat:opacity-100 md:focus:opacity-100"><FolderInput className="size-4 md:size-3.5" /></button><button type="button" onClick={() => setDeleteConversation(conversation)} aria-label={`Excluir ${conversation.title ?? "chat"}`} className="rounded p-2 text-muted-foreground hover:bg-destructive/10 hover:text-destructive md:p-1 md:opacity-0 md:group-hover/chat:opacity-100 md:focus:opacity-100"><Trash2 className="size-4 md:size-3.5" /></button></div>;
  }
  ...
  return <div className="flex flex-1 flex-col overflow-hidden">
    ...
    <nav className="flex-1 overflow-y-auto px-2 pb-2">
      ...
      <div className="space-y-1">{projects.map((project) => { const items = filtered.filter((conversation) => conversation.projectId === project.id); const isCollapsed = collapsed.has(project.id); return <section key={project.id} {...dropProps(project.id, project.id)} className={cn("rounded-lg border border-transparent p-1 transition-colors", dropTarget === project.id && "border-primary bg-primary/10")}><div className="group/project flex items-center gap-1"><button type="button" onClick={() => toggleProject(project.id)} className="rounded p-2 text-muted-foreground hover:bg-accent md:p-1" aria-label={isCollapsed ? `Expandir ${project.name}` : `Recolher ${project.name}`}>{isCollapsed ? <ChevronRight className="size-4 md:size-3.5" /> : <ChevronDown className="size-4 md:size-3.5" />}</button><Link href={`/projetos/${project.id}`} className="min-w-0 flex-1 truncate text-sm font-medium"><Folder className="mr-1.5 inline size-4 text-primary" />{project.name}</Link><span className="text-[10px] text-muted-foreground">{items.length}</span><Link href={`/?project=${project.id}`} className="rounded px-2 py-1 text-base leading-none text-muted-foreground hover:text-primary md:px-1 md:py-0 md:opacity-0 md:group-hover/project:opacity-100" aria-label={`Novo chat em ${project.name}`}>+</Link><button type="button" onClick={() => setDeleteProject(project)} className="rounded p-2 text-muted-foreground hover:text-destructive md:p-1 md:opacity-0 md:group-hover/project:opacity-100 md:focus:opacity-100" aria-label={`Excluir projeto ${project.name}`}><Trash2 className="size-4 md:size-3" /></button></div>{!isCollapsed && <div className="mt-1 ml-2 space-y-0.5 border-l border-border/60 pl-2">{items.map(conversationItem)}{items.length === 0 && <p className="px-2 py-2 text-xs text-muted-foreground"><span className="md:hidden">Use o ícone de pasta em um chat para movê-lo para cá</span><span className="hidden md:inline">Arraste um chat para cá</span></p>}</div>}</section>; })}</div>
    ...
    <Dialog open={Boolean(moveConversationTarget)} onOpenChange={(open) => { if (!open) setMoveConversationTarget(null); }}><DialogContent><DialogHeader><DialogTitle>Mover chat</DialogTitle><DialogDescription>Escolha onde guardar “{moveConversationTarget?.title ?? "Nova conversa"}”.</DialogDescription></DialogHeader><div className="grid gap-2"><Button variant={!moveConversationTarget?.projectId ? "secondary" : "outline"} className="h-auto justify-start py-3" disabled={busy === moveConversationTarget?.id || !moveConversationTarget?.projectId} onClick={() => void moveConversation(null, moveConversationTarget?.id)}><MessageSquare />Chats avulsos</Button>{projects.map((project) => <Button key={project.id} variant={moveConversationTarget?.projectId === project.id ? "secondary" : "outline"} className="h-auto justify-start py-3" disabled={busy === moveConversationTarget?.id || moveConversationTarget?.projectId === project.id} onClick={() => void moveConversation(project.id, moveConversationTarget?.id)}><Folder />{project.name}</Button>)}</div></DialogContent></Dialog>

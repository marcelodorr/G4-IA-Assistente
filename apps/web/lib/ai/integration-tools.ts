...
export async function createIntegrationTools(db: Db, owner: Owner, options?: { beforeCall?: () => void }): Promise<ToolSet> {
  const integrations = await listUserIntegrations(db, owner.userId);
  const connected = new Set(integrations.filter((item) => item.connected).map((item) => item.id));

...
describe("GitBook client", () => {
  it("pesquisa conteúdo usando Bearer token e parâmetros seguros", async () => {
    const fetchMock = vi.fn<typeof fetch>(async () => Response.json({ count: 1, items: [{ title: "Manual MES" }] }));
  ...
  it("lê página em Markdown e rejeita identificadores inválidos", async () => {
    const fetchMock = vi.fn<typeof fetch>(async () => Response.json({ id: "page_1", markdown: "# Manual" }));
    vi.stubGlobal("fetch", fetchMock);
